...
├── utils/                         # + 工具
│   └── icons.ts                   # + 图标映射表（meta.icon 字符串 → EP 组件，集中登记）
...
> 注：本套截图里没有独立的 `acl/permission`（菜单管理）页面截图。分配权限功能以弹窗形式集成在角色管理页面中。
...
> - **`meta.icon` 是字符串名字**（如 `'Goods'`），不是组件。名字到组件的绑定在 `src/utils/icons.ts` 的 `routeIconMap` 完成，菜单/面包屑用 `getRouteIcon(name)` 查表取组件，路由里无需 `import` 图标。

---

## 四、步骤 4：集中登记图标映射表 `src/utils/icons.ts`

路由表里的 `meta.icon` 只是字符串名字，需要一个地方把它绑定到真正的 `@element-plus/icons-vue` 组件——这就是 `src/utils/icons.ts`，作为路由骨架阶段的基础设施与路由表配套。

```ts
// src/utils/icons.ts
import {
  Files, Goods, Histogram, HomeFilled,
  Lock, Menu as MenuIcon, Stamp, UserFilled,
...
export const routeIconMap: Record<string, Component> = {
  ...
  Menu: MenuIcon, Goods, Stamp, Files,
}

// meta.icon 是字符串（如 'HomeFilled'），查表转成组件；空则返回 undefined 不渲染图标
...
- `routeIconMap` 的 key 必须和路由 `meta.icon` 的字符串**完全一致**（当前 8 个：`HomeFilled`/`Histogram`/`Lock`/`UserFilled`/`Menu`/`Goods`/`Stamp`/`Files`）。key 对不上 → `getRouteIcon` 返回 `undefined` → 图标空白。
- 每个图标都要在文件顶部**显式 `import`** 一次——这是 `@element-plus/icons-vue` 按需加载的硬性要求（打包工具只认静态 `import` 语句，扫不到运行时的字符串名）。所以新增图标时两步都不能少：① 路由写 `icon: 'Xxx'`；② 这里 `import { Xxx }` 并加进 `routeIconMap`。
- 菜单/面包屑只 `import { getRouteIcon }` 引用，不直接碰图标组件，保证全站图标来源统一。

---

## 五、步骤 5：创建路由实例 `src/router/index.ts`
...
// 图标统一在 utils/icons.ts 登记（集中 import EP 图标 + 名字→组件映射），菜单只引用
...
// meta.icon 是字符串（如 'HomeFilled'），查共用映射表转成组件；空则返回 undefined 不渲染图标
...
**③ 图标为什么要集中登记？** 菜单图标是 `meta.icon: 'HomeFilled'` 这种**字符串**，靠 `<component :is>` 运行时查组件；而 vite 自动导入只认模板里写死的标签、扫不到字符串名，所以必须在 `utils/icons.ts` **手动 import** 图标、用 `routeIconMap` 做"字符串→组件"的映射表（`meta.icon` 用到的名字都要登记，否则图标不显示）。菜单/面包屑只 `import { getRouteIcon }` 引用，新增图标只改这一处。
