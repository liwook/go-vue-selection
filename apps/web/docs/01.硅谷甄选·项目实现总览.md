...
- **阶段 8 菜单/权限管理**：菜单树（`el-table` 树形）+ 增删改。文件：`src/views/acl/permission/index.vue`【新建】、`src/api/acl/permission/*`【新建】
...
- 数据来源：对接真实后端，`/admin/acl/user/info` 返回的 `userInfo.buttons` 数组即按钮权限；`admin` 账号返回全部标识。
